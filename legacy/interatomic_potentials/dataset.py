        ...
        for name in "energies forces stresses magmoms structure_ids".split():
            ...
            if labels is not None and len(labels) != len(structures):
                raise RuntimeError(
                    ...
                    f"{len(structures)!r}, len({name})={len(labels)}"
                )
    ...
    def from_vasp(
        ...
        *,
        ...
        return cls(
            ...
            stresses=(
                None if result_dict["stress"] in [None, []] else result_dict["stress"]
            ),
            magmoms=(
                None if result_dict["magmom"] in [None, []] else result_dict["magmom"]
            ),
...
class CIFData(paddle.io.Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        self.data = io.read_json(os.path.join(cif_path, labels))
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(labels, str):
            ...
            self.labels = io.read_json(labels)
        elif isinstance(labels, dict):
            self.labels = labels
        if isinstance(exclude, str):
            self.excluded_graph = io.read_json(exclude)
            ...
            if os.path.isdir(data):
                for json_path in os.listdir(data):
                    if json_path.endswith(".json"):
                        ...
                        self.data.update(io.read_json(os.path.join(data, json_path)))
            else:
                print(f"Importing: {data}")
                self.data.update(io.read_json(data))
