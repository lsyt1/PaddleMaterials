...
class BasicMolecularMetrics(object):
    ...
    def compute_validity(self, generated):
        ...
        for graph in generated:
            ...
            mol = build_molecule(atom_types, edge_types, self.dataset_info.
                atom_decoder)
            ...
            if smiles is not None:
                try:
                    mol_frags = Chem.rdmolops.GetMolFrags(mol, asMols=True,
                        sanitizeFrags=True)
                    largest_mol = max(mol_frags, default=mol, key=lambda m:
                        m.GetNumAtoms())
                ...
                all_smiles.append(None)
        return valid, len(valid) / len(generated), np.array(num_components
            ), all_smiles
    ...
    def compute_relaxed_validity(self, generated):
        ...
        for graph in generated:
            atom_types, edge_types = graph
            mol = build_molecule_with_partial_charges(atom_types,
                edge_types, self.dataset_info.atom_decoder)
            smiles = mol2smiles(mol)
            if smiles is not None:
                try:
                    mol_frags = Chem.rdmolops.GetMolFrags(mol, asMols=True,
    ...
    def evaluate(self, generated):
        ...
        if relaxed_validity > 0:
            ...
            uniqueness = 0.0
            unique = []
        return [validity, relaxed_validity, uniqueness, novelty], unique, dict(
            nc_min=nc_min, nc_max=nc_max, nc_mu=nc_mu), all_smiles
...
def build_molecule(atom_types, edge_types, atom_decoder, verbose=False):
    ...
    for i, bond in enumerate(all_bonds):
        if bond[0].item() != bond[1].item():
            mol.AddBond(bond[0].item(), bond[1].item(), bond_dict[
                edge_types[bond[0], bond[1]].item()])
            if verbose:
                print('bond added:', bond[0].item(), bond[1].item(),
                    edge_types[bond[0], bond[1]].item(), bond_dict[
                    edge_types[bond[0], bond[1]].item()])
    return mol


def build_molecule_with_partial_charges(atom_types, edge_types,
    atom_decoder, verbose=False):
    if verbose:
    ...
    while True:
        ...
        if flag:
            ...
            for b in mol.GetAtomWithIdx(idx).GetBonds():
                ...
                queue.append((b.GetIdx(), type, b.GetBeginAtomIdx(), b.
                    GetEndAtomIdx()))
                if type == 12:
                    check_idx += 1
...
def compute_molecular_metrics(molecule_list, train_smiles, dataset_info):
    ...
    if not dataset_info.remove_h:
        ...
        for i, mol in enumerate(molecule_list):
            ...
            validity_results = check_stability(atom_types, edge_types,
                dataset_info)
        ...
        validity_dict = {'mol_stable': fraction_mol_stable, 'atm_stable':
            fraction_atm_stable}
    else:
