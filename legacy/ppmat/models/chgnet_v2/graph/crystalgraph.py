...
class CrystalGraph:
    ...
    def __init__(
        ...
        composition_fea: paddle.Tensor | None = None,
    ) -> None:
        ...
        composition_fea = paddle.bincount(x=atomic_number - 1, minlength=94)
        composition_fea = composition_fea / atomic_number.shape[0]
        self.composition_fea = composition_fea
        ...
        self.atom_cart_coords = atom_frac_coord @ lattice

        center = self.atom_cart_coords[self.atom_graph[:, 0]]
        neighbor = self.atom_cart_coords[self.atom_graph[:, 1]]

        neighbor = neighbor + neighbor_image @ lattice
        ...
        self.bond_vectors = bond_vectors
        self.undirected_bond_lengths = undirected_bond_lengths

        self.atomic_numbers = None

        bond_vecs_i = paddle.gather(x=bond_vectors, axis=0, index=self.bond_graph[:, 2])
        bond_vecs_j = paddle.gather(x=bond_vectors, axis=0, index=self.bond_graph[:, 4])

        self.bond_vecs_i = bond_vecs_i
        self.bond_vecs_j = bond_vecs_j
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"CrystalGraph(composition={composition!r}, "
            f"atom_graph_cutoff={atom_graph_cutoff!r}, "
            f"bond_graph_cutoff={bond_graph_cutoff!r}, "
            f"n_atoms={n_atoms!r}, "
            f"atom_graph_len={atom_graph_len!r}, "
            f"bond_graph_len={bond_graph_len!r})"
        )
