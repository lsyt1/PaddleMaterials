...
from ppmat.models.chgnet.graph.graph import Graph
from ppmat.models.chgnet.graph.graph import Node
...
try:
    from ppmat.models.chgnet.graph.cygraph import make_graph
...
class CrystalGraphConverter(paddle.nn.Layer):
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"{cls_name}(algorithm={algorithm!r}, "
    ...
    def forward(self, structure: Structure, graph_id=None, mp_id=None) -> CrystalGraph:
        ...
        atomic_number = paddle.to_tensor(data, dtype="int32", stop_gradient=True)

        atom_frac_coord = paddle.to_tensor(
            data=structure.frac_coords, dtype="float32", stop_gradient=False
        )
        lattice = paddle.to_tensor(
            data=structure.lattice.matrix, dtype="float32", stop_gradient=False
        )
        ...
        if n_isolated_atoms:
            atom_graph_cutoff = self.atom_graph_cutoff
            msg = (
                f"Structure graph_id={graph_id!r} has {n_isolated_atoms} isolated "
                f"atom(s) with atom_graph_cutoff={atom_graph_cutoff!r}. "
                "CHGNet calculation will likely go wrong"
            )
        ...
        return CrystalGraph(
            ...
            neighbor_image=paddle.to_tensor(data=image, dtype="float32"),
