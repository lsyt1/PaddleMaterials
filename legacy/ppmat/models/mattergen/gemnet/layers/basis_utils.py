...
def associated_legendre_polynomials(L_maxdegree, zero_m_only=True, pos_m_only=True):
    """Computes string formulas of the associated legendre polynomials up to degree
    L (excluded).
...
def real_sph_harm(L_maxdegree, use_theta, use_phi=True, zero_m_only=True):
    ...
    degree L (excluded).
    Variables are either spherical coordinates phi and theta (or cartesian
    ...
    if zero_m_only:
        ...
        Y_l_m = [([0] * (2 * l_degree + 1)) for l_degree in range(L_maxdegree)]
    if use_theta:
