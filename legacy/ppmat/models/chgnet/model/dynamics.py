...
class CHGNetCalculator(Calculator):
    ...
    def calculate(
        ...
        model_prediction = self.model.predict_graph(
            graph, task="efsm", return_crystal_feas=True
        )
        ...
        **kwargs,
        ...
        if isinstance(ase_filter, str):
            if ase_filter in valid_filter_names:
                ...
                raise ValueError(
                    f"Invalid ase_filter={ase_filter!r}, must be one of "
                    f"{valid_filter_names}. "
                )
        ...
        if ensemble.lower() == "nve":
            ...
            if bulk_modulus is not None:
                ...
                try:
                    ...
                    print(
                        f"Completed bulk modulus calculation: k = {bulk_modulus:.3}GPa,"
                        f" {bulk_modulus_au:.3}eV/A^3"
                    )
                except Exception:
                    ...
                    warnings.warn(
                        ...
                        "pressure relaxation time.User input for bulk modulus is "
                        "recommended.",
                        stacklevel=2,
                    )
