...
def combine_array_with_time(x: np.ndarray, t: Tuple[int, ...]) -> np.ndarray:
    ...
    this function will repeat t_i for N times and will concat it with data x for
    each t_i in t, finally return the stacked result, which is of shape (N×T, D+1).
...
def plot_curve(
    ...
    Args:
        ...
        smooth_step (int, optional): How many points are squeezed to one point to
            smooth the curve. Defaults to 1.
        use_semilogy (bool, optional): Whether to set non-uniform coordinates for the
