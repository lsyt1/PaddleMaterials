...
class MatterGenWithGuidance(paddle.nn.Layer):
    def __init__(
        self,
        decoder_cfg,
        beta_scheduler_cfg,
    ...
    @paddle.no_grad()
    def sample(self, batch, step_lr=1e-07, guide_w=0.5):
        batch_size = batch["num_graphs"]
        ...
        for t in tqdm(range(time_start, 0, -1)):
            ...
            pred_a = (1 + guide_w) * pred_a[: batch["num_nodes"]] - guide_w * pred_a[
                batch["num_nodes"] :
            ]
