...
class MatterGen(paddle.nn.Layer):
    ...
    def forward(self, batch):
        ...
        # P = vecV.transpose([0, 2, 1]).multiply(vals.view([vals.shape[0], 1, vals.shape[1]])) @ vecV
        # U = vecU @ vecV

        # rand_l = paddle.randn(shape=lattices.shape, dtype=lattices.dtype)
        # # P = lattice
        # rand_l = rand_l.tril() + rand_l.tril(diagonal=-1).transpose([0, 2, 1])
        # # P = c0[:, None, None] * P  + ((1 - c0) * (batch['num_atoms']/20)**(1/3))[:, None, None] + c1[:, None, None] * rand_l * ((batch['num_atoms']*199)**(1/3))[:, None, None]
