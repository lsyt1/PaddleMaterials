...
class CSPDiffusionPP(paddle.nn.Layer):
    ...
    def forward(self, batch):
        ...
        #lattices = paddle.to_tensor(np.load('/home/liuxuwei01/DiffCSP-PP/lattices.npy'))
        frac_coords = batch["frac_coords"]
        
        rand_x = paddle.randn(shape=frac_coords.shape, dtype=frac_coords.dtype)
        #rand_x = paddle.to_tensor(np.load('/home/liuxuwei01/DiffCSP-PP/rand_x.npy'))
        sigmas_per_atom = sigmas.repeat_interleave(repeats=batch["num_atoms"])[:, None]
        sigmas_norm_per_atom = sigmas_norm.repeat_interleave(repeats=batch["num_atoms"])[:, None]
        
        rand_x_anchor = rand_x[batch["anchor_index"]]
        rand_x_anchor = (batch["ops_inv"][batch["anchor_index"]] @ rand_x_anchor.unsqueeze(axis=-1)).squeeze(axis=-1)
        rand_x_anchor = paddle.cast(rand_x_anchor, dtype='float32')
        rand_x = (batch["ops"][:, :3, :3] @ rand_x_anchor.unsqueeze(axis=-1)).squeeze(axis=-1)
        ...
        #rand_crys_fam = paddle.to_tensor(np.load('/home/liuxuwei01/DiffCSP-PP/rand_crys_fam.npy')) 
        ...
        #input_frac_coords = paddle.to_tensor(np.load('/home/liuxuwei01/DiffCSP-PP/input_frac_coords.npy'))
        #input_crys_fam = paddle.to_tensor(np.load('/home/liuxuwei01/DiffCSP-PP/input_crys_fam.npy'))
        ...
        tar_x_anchor = d_log_p_wrapped_normal(sigmas_per_atom * rand_x_anchor, sigmas_per_atom) / paddle.sqrt(x=sigmas_norm_per_atom)
        
        loss_lattice = paddle.nn.functional.mse_loss(input=pred_crys_fam, label=rand_crys_fam)
        loss_coord = paddle.nn.functional.mse_loss(input=pred_x_proj, label=tar_x_anchor)
    ...
    def sample(self, batch, diff_ratio = 1.0, step_lr=1e-05):
        ...
        if diff_ratio < 1:
            ...
            rand_crys_fam, rand_x = paddle.randn(shape=ori_crys_fam.shape, dtype=ori_crys_fam.dtype), paddle.randn(shape=frac_coords.shape, dtype=frac_coords.dtype)
            ...
            rand_x_anchor = rand_x[batch["anchor_index"]]
            rand_x_anchor = (batch.ops[batch.anchor_index, :3, :3] @ rand_x_anchor.unsqueeze(axis=-1)).squeeze(axis=-1)
            rand_x = (batch.ops[:, :3, :3] @ rand_x_anchor.unsqueeze(axis=-1)).squeeze(axis=-1)
        ...
        x_T_all = paddle.concat(x=[x_T[batch.anchor_index], paddle.ones(shape=[batch.ops.size(0), 1]).to(x_T.place)], axis=-1).unsqueeze(axis=-1)
        
        x_T = (batch.ops @ x_T_all).squeeze(axis=-1)[:, :3] % 1.0
        
        traj = {time_start: {'num_atoms': batch.num_atoms, 'atom_types':batch.atom_types, 'frac_coords': x_T % 1.0, 'lattices': l_T,'crys_fam': crys_fam_T}}
        ...
        for t in tqdm(range(time_start, 0, -1)):
            ...
            rand_x = paddle.randn(shape=x_T.shape, dtype=x_T.dtype) if t > 1 else paddle.zeros_like(x=x_T)
            ...
            pred_x = (batch.ops[:, :3, :3] @ pred_x_anchor.unsqueeze(axis=-1)).squeeze(axis=-1)
            
            x_t_minus_05 = x_t - step_size * pred_x + std_x * rand_x
            crys_fam_t_minus_05 = crys_fam_t
            
            frac_coords_all = paddle.concat(x=[x_t_minus_05[batch.anchor_index], paddle.ones(shape=[batch.ops.size(0), 1]).to(x_t_minus_05.place)], axis=-1).unsqueeze(axis=-1)
            
            x_t_minus_05 = (batch.ops @ frac_coords_all).squeeze(axis=-1)[:, :3] % 1.0
            ...
            std_x = paddle.sqrt(x=adjacent_sigma_x ** 2 * (sigma_x ** 2 - adjacent_sigma_x ** 2) / sigma_x ** 2)
            ...
            frac_coords_all = paddle.concat(x=[x_t_minus_1[batch.anchor_index], paddle.ones(shape=[batch.ops.size(0), 1]).to(x_t_minus_1.place)], axis=-1).unsqueeze(axis=-1)
            
            x_t_minus_1 = (batch.ops @ frac_coords_all).squeeze(axis=-1)[:, :3] % 1.0
            
            traj[t - 1] = {'num_atoms': batch.num_atoms, 'atom_types':batch.atom_types, 'frac_coords': x_t_minus_1 % 1.0,'lattices': l_t_minus_1, 'crys_fam': crys_fam_t_minus_1}
        
        traj_stack = {
            'num_atoms': batch["num_atoms"], 
            'atom_types': batch["atom_types"], 
            'all_frac_coords': paddle.stack(x=[traj[i]['frac_coords'] for i in range(time_start, -1, -1)]),
            'all_lattices': paddle.stack(x=[traj[i]['lattices'] for i in range(time_start, -1, -1)])
        }
