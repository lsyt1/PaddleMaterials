...
def get_unique(
    structure_matcher: StructureMatcher, structures: List[Structure]
) -> List[int]:
    if len(structures) == 1:
        return [0]
...
def get_global_index_from_local_index(
    ...
    global_indices = [
        entries_mapping_by_key[k][vv].entry_id
    ...
    for k, match_dict in local_index.items():
        ...
        for d1_ix, ref_ix_list in match_dict.items():
            global_match_dict[data_entries_mapping[d1_ix].entry_id] = [
                reference_entries_mapping[match_ix].data["material_id"]
                for match_ix in ref_ix_list
            ]
...
def get_mask_from_local_index(
    ...
    """Turn local structure chemsys index into global structure mask."""
    global_indices = get_global_index_from_local_index(
        entries_mapping_by_key, local_index
    )
...
class OrderedDatasetUniquenessComputer:
    def __init__(
        self, structure_matcher: StructureMatcher = DefaultDisorderedStructureMatcher()
    ):
    ...
    def __call__(self, dataset: ReferenceDataset) -> np.typing.NDArray[bool]:
        ...
        for reduced_formula, data_entries in tqdm(
            ...
            local_index[reduced_formula] = get_unique(
                self.structure_matcher, structures
            )
        return get_mask_from_local_index(
            dataset.entries_by_reduced_formula, local_index
        )
    ...
    def __call__(self, dataset: "ReferenceDataset") -> np.typing.NDArray[bool]:
        ...
        for chemsys, data_entries in tqdm(
            ...
            if not all([s.is_ordered for s in structures]):
                logger.warning(
                    "Using DisorderedDatasetUniquenessComputer for ordered structures. This is less efficient than using OrderedDatasetUniquenessComputer."
                )
...
def matches_to_mask(
    match_idx: Iterable[int], num_samples: int
...
class DatasetMatcher:
    ...
    def __call__(
        ...
        grouped_reference_entries = self.grouped_dataset_entries(
            dataset=reference_dataset
        )
        for group_key, data_entries in tqdm(
            grouped_dataset_entries.items(), desc="Finding novel structures"
        ):
            ...
            matches = get_matches(
                self.structure_matcher, data_structures, reference_structures
            )
