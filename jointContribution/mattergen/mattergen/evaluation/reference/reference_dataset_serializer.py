...
import lmdb
from mattergen.evaluation.reference.reference_dataset import (
    ReferenceDataset, ReferenceDatasetImpl)
from mattergen.evaluation.utils.lmdb_utils import (lmdb_get, lmdb_open,
                                                   lmdb_read_metadata)
...
def gzip_compress(
    file_path: (str | os.PathLike), output_dir: (str | os.PathLike)
) -> Path:
...
def gzip_decompress(
    gzip_file_path: (str | os.PathLike), output_dir: (str | os.PathLike)
) -> Path:
...
def lmdb_put(txn: lmdb.Transaction, key: str, value: Any) -> bool:
    ...
    """
    return txn.put(
        key.encode("ascii"), pickle.dumps(value, protocol=pickle.HIGHEST_PROTOCOL)
    )
...
class LMDBGZSerializer:
    ...
    def serialize(
        self, ref_dataset: ReferenceDataset, dataset_path: (str | os.PathLike)
    ) -> None:
        """Writes a dataset to a file using the gzip-compressed LMDB format."""
        lmdb_file_path = str(dataset_path)[:-3]
        with lmdb_open(lmdb_file_path, readonly=False) as env:
            ...
            for entry in tqdm(
                ref_dataset, desc="Serializing dataset", total=len(ref_dataset)
            ):
                entry.structure.unset_charge()
                structure_without_oxidation_states = (
                ...
                chemsys = "-".join(
                    sorted({el.symbol for el in entry.composition.elements})
                )
    ...
    def deserialize(self, dataset_path: (str | os.PathLike)) -> ReferenceDataset:
        ...
        return ReferenceDataset(
            name=name, impl=LMDBBackedReferenceDatasetImpl(lmdb_path, cleanup_dir=True)
        )
...
class LMDBBackedReferenceDatasetImpl(ReferenceDatasetImpl):
    ...
    def __init__(self, lmdb_path: Path, cleanup_dir: bool = False):
        ...
        self.total_num_entries = sum(
            sum(d.values())
            for d in self.num_entries_by_chemsys_reduced_formulas.values()
        )
    ...
    def _build_num_entries_by_chemsys_reduced_formulas(
        ...
        with self.env.begin() as txn:
            for chemsys in chemical_systems:
                reduced_formulas = lmdb_read_metadata(
                    lmdb_path, f"{chemsys}.reduced_formulas"
                )
    ...
    def __iter__(self) -> Iterator[ComputedStructureEntry]:
        ...
        for (
            ...
            for reduced_formula in num_entries_by_reduced_formula:
                yield from self.get_entries_by_chemsys_reduced_formula(
                    chemsys, reduced_formula
                )
    ...
    def get_entries_by_chemsys(self, chemsys: str) -> Iterator[ComputedStructureEntry]:
        for reduced_formula in self.num_entries_by_chemsys_reduced_formulas[
            chemsys
        ].keys():
            yield from self.get_entries_by_chemsys_reduced_formula(
                chemsys, reduced_formula
...
class LMDBBackedChemicalSystemLookup(
    WeakRefImplMixin, Mapping[str, list[ComputedStructureEntry]]
):
...
class LMDBBackedReducedFormulaLookup(
    WeakRefImplMixin, Mapping[str, list[ComputedStructureEntry]]
):
