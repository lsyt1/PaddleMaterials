...
def get_set_of_all_elements(
    ...
    return set(
        str(element)
        for x in structure_summaries
        for element in x.entry.composition.elements
    )
...
class MissingTerminalsAndEnergy:
    ...
    def from_dataset_and_reference(
        ...
        missing_terminals = list(
            terminal_systems - set(reference.entries_by_chemsys.keys())
        )
        terminals_in_reference = terminal_systems & set(
            reference.entries_by_chemsys.keys()
        )
        missing_energy = [
            chemsys
...
class EnergyMetricsCapability(BaseMetricsCapability):
    ...
    def check_missing_reference_terminal_systems(
        ...
        return MissingTerminalsAndEnergy.from_dataset_and_reference(
            structure_summaries=structure_summaries, reference=reference_dataset
        )
    ...
    def warn_missing_data(cls, missing_terminals: MissingTerminalsAndEnergy) -> None:
        ...
        if missing_terminals.has_missing_terminals:
            logger.warning(
                f"Missing terminal systems: {missing_terminals.missing_terminals}"
            )
    ...
    def __init__(
        ...
        super().__init__(
            structure_summaries=structure_summaries, n_failed_jobs=n_failed_jobs
        )
    ...
    @lru_cache
    def _get_self_consistent_energy_above_hull_per_atom_chemsys(
        self, chemsys: str
    ) -> list[float]:
...
class AvgRMSDFromRelaxation(BaseEnergyMetric, BaseAggregateMetric):
    ...
    def compute_pre_aggregation_values(self) -> numpy.typing.NDArray:
        return np.array(
            [
                d.rmsd_from_relaxation
                for d in self.energy_capability._structure_summaries
            ]
        )
...
class FracStableStructures(BaseEnergyMetric, BaseAggregateMetric):
    ...
    def value(self) -> float:
        return (
            self.pre_aggregation_values.sum()
            / self.energy_capability.total_submitted_jobs
        )
