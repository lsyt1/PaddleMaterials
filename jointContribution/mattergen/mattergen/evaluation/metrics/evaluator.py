...
from mattergen.evaluation.utils.metrics_structure_summary import (
    MetricsStructureSummary, get_metrics_structure_summaries)
...
def unique_item(iterable: Iterable[T]) -> T:
    ...
    assert (
        len(lst) == 1
    ), f"Tried to call unique_item, but {lst} contains {len(lst)} items."
    return lst[0]
...
class MetricsEvaluator:
    ...
    def from_structures(
        ...
        if reference is None:
            print(
                "No reference dataset provided. Using MP2020 correction dataset as reference."
            )
            reference = ReferenceMP2020Correction()
        structure_summaries = [
            MetricsStructureSummary.from_structure(s) for s in structures
        ]
    ...
    def from_structures_and_energies(
        ...
        if reference is None:
            print(
                "No reference dataset provided. Using MP2020 correction as reference."
            )
            reference = ReferenceMP2020Correction()
    ...
    def available_metrics(self) -> list[Type[BaseMetric]]:
        return [
            ...
            if all(
                cap in self.available_capability_types
                for cap in metric.required_capabilities
            )
        ]
    ...
    def _get_capability(self, capability: Type[CapabilityT]) -> CapabilityT:
        ...
        return unique_item(
            cap for cap in self.capabilities if isinstance(cap, capability)
        )
    ...
    def as_dataframe(
        ...
        df = DataFrame(
            data=data, index=[e.entry_id for e in self.capabilities[0].dataset]
        )
        dfs = [df] + [cap.as_dataframe() for cap in self.capabilities]
        assert all(
            [(len(df) == len(d)) for d in dfs]
