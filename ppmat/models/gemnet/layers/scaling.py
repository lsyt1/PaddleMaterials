...
class AutomaticFit:
    ...
    def _add2queue(self):
        ...
        for var in AutomaticFit.queue:
            if self._name == var._name:
                raise ValueError(
                    f"Variable with the same name ({self._name}) was already added "
                    "to queue!"
                )
    ...
    def load_maybe(self):
        ...
        if value is None:
            print(f"Initialize variable {self._name}' to {self.variable.numpy():.3f}")
        else:
            self._fitted = True
            print(f"Set scale factor {self._name} : {value}")
            with paddle.no_grad():
    ...
    def fit(self):
        ...
        if AutomaticFit.activeVar == self:
            ...
            print(
                ...
                f"Var_out:{self.variance_out.item():.3f}, Ratio: {ratio.item():.3f}=> "
                f"Scaling factor: {value.item():.3f}"
            )
