...
// in the graph class in chgnet.graph.graph such that anyone familiar with that
// code should be able to pick up this code pretty easily.
...
typedef struct _Node {
  ...
  LongToDirectedEdgeList
      *neighbors; // Assuming neighbors can only be directed edge. Key is
                  // dest_node, value is DirectedEdge struct
...
bool find_in_undirected(NodeIndexPair *tmp,
                        StructToUndirectedEdgeList **undirected_edges,
                        StructToUndirectedEdgeList **found_entry);
void directed_to_undirected(DirectedEdge *directed, UndirectedEdge *undirected,
                            int64_t undirected_index);
void create_new_undirected_edges_entry(
    StructToUndirectedEdgeList **undirected_edges, NodeIndexPair *tmp,
    UndirectedEdge *new_undirected_edge);
void append_to_undirected_edges_tmp(
    UndirectedEdge *undirected, StructToUndirectedEdgeList **undirected_edges,
    NodeIndexPair *tmp);
void append_to_undirected_edges_list(UndirectedEdge **undirected_edges_list,
                                     UndirectedEdge *to_add,
                                     int64_t *num_undirected_edges);
void append_to_directed_edges_list(DirectedEdge **directed_edges_list,
                                   DirectedEdge *to_add,
                                   int64_t *num_directed_edges);
void add_neighbors_to_node(Node *node, int64_t neighbor_index,
                           DirectedEdge *directed_edge);
void print_neighbors(Node *node);
void append_to_directed_edge_indices(UndirectedEdge *undirected_edge,
                                     int64_t directed_edge_index);
bool is_reversed_directed_edge(DirectedEdge *directed_edge1,
                               DirectedEdge *directed_edge2);
...
ReturnElems2 *
create_graph(int64_t *center_indices, int64_t num_edges,
             int64_t *neighbor_indices,
             int64_t *images, // contiguous memory (row-major) of image elements
                              // (total of n_e * 3 integers)
             double *distances, int64_t num_atoms) {
  ...
  UndirectedEdge **undirected_edges_list =
      calloc(num_edges, sizeof(UndirectedEdge));
  int64_t num_undirected_edges = 0;
  StructToUndirectedEdgeList *undirected_edges = NULL;

  // Pointer to beginning of list of UndirectedEdges corresponding to tmp of
  // current iteration
  ...
  for (int64_t i = 0; i < num_edges; i++) {
    ...
    found =
        find_in_undirected(tmp, &undirected_edges, &corr_undirected_edges_item);
    ...
    if (!found) {
      ...
      directed_to_undirected(this_directed_edge, this_undirected_edge,
                             num_undirected_edges);

      // Add this new edge information to various data structures
      create_new_undirected_edges_entry(&undirected_edges, tmp,
                                        this_undirected_edge);
      append_to_undirected_edges_list(
          undirected_edges_list, this_undirected_edge, &num_undirected_edges);
      add_neighbors_to_node(&nodes[center_indices[i]], neighbor_indices[i],
                            this_directed_edge);
      append_to_directed_edges_list(directed_edges_list, this_directed_edge,
                                    &num_directed_edges);
    } else {
      // This pair of nodes has been added before. We have to check if it's the
      // other directed edge (but pointed in the different direction) OR it's
      // another totally different undirected edge that has different image and
      // distance

      // if found is true, then corr_undirected_edges_item points to
      // self.undirected_edges[tmp] iterate through all previously scanned
      // undirected edges that have the same endpoints as this edge if there
      // exists an undirected edge with the same inverted image as
      // this_undirected_edge, then add this new directed edge and associate it
      // with this undirected edge
      for (int j = 0;
           j < corr_undirected_edges_item->num_undirected_edges_in_group; j++) {
        // Grab the 0th directed edge associated with this undirected edge
        added_DE = directed_edges_list[(
            (corr_undirected_edges_item->undirected_edges_list)[j]
                ->directed_edge_indices)[0]];

        if (is_reversed_directed_edge(added_DE, this_directed_edge)) {
          this_directed_edge->undirected_edge_index =
              added_DE->undirected_edge_index;
          add_neighbors_to_node(&nodes[center_indices[i]], neighbor_indices[i],
                                this_directed_edge);
          append_to_directed_edges_list(directed_edges_list, this_directed_edge,
                                        &num_directed_edges);
          append_to_directed_edge_indices(
              (corr_undirected_edges_item->undirected_edges_list)[j],
              this_directed_edge->index);
          processed_edge = true;
          break;
        }
      }
      // There wasn't a pre-existing undirected edge that corresponds to this
      // directed edge Create a new undirected edge and process
      if (!processed_edge) {
        this_directed_edge->undirected_edge_index = num_undirected_edges;
        ...
        directed_to_undirected(this_directed_edge, this_undirected_edge,
                               num_undirected_edges);
        append_to_undirected_edges_tmp(this_undirected_edge, &undirected_edges,
                                       tmp);
        append_to_undirected_edges_list(
            undirected_edges_list, this_undirected_edge, &num_undirected_edges);
        add_neighbors_to_node(&nodes[center_indices[i]], neighbor_indices[i],
...
// NOTE: assumes that directed_edge1->center = directed_edge2->neighbor and
// directed_edge1->neighbor = directed_edge2->center
bool is_reversed_directed_edge(DirectedEdge *directed_edge1,
                               DirectedEdge *directed_edge2) {
  for (int i = 0; i < 3; i++) {
  ...
  // The two directed edges should have opposing center/neighbor nodes (i.e.
  // center-neighbor for DE1 is [0, 1] and for DE2 is [1, 0]) We check for that
  // condition here
...
// If tmp or the reverse of tmp is found in undirected_edges, True is returned
// and the corresponding StructToUndirectedEdgeList pointer is placed into
// found_entry. Otherwise, False is returned. NOTE: does not edit the *tmp
...
void create_new_undirected_edges_entry(
    StructToUndirectedEdgeList **undirected_edges, NodeIndexPair *tmp,
    UndirectedEdge *new_undirected_edge) {
  StructToUndirectedEdgeList *new_entry =
      malloc(sizeof(StructToUndirectedEdgeList));
...
// Appends undirected into the StructToUndirectedEdgeList entry that corresponds
// to tmp This function will first look up tmp
void append_to_undirected_edges_tmp(
    UndirectedEdge *undirected, StructToUndirectedEdgeList **undirected_edges,
    NodeIndexPair *tmp) {
  ...
  int64_t num_undirected_edges =
      this_undirected_edges_item->num_undirected_edges_in_group;

  // No need to worry about originally malloc'ing memory for
  // this_undirected_edges_item->undirected_edges_list this is because, we first
  // call create_new_undirected_edges_entry for all entries. This function
  // already mallocs for us.

  // Realloc the space to fit a new pointer to an undirected edge
  UndirectedEdge **new_list =
      realloc(this_undirected_edges_item->undirected_edges_list,
              sizeof(UndirectedEdge *) * (num_undirected_edges + 1));
  this_undirected_edges_item->undirected_edges_list = new_list;

  // Insert the undirected pointer into the newly allocated slot
  this_undirected_edges_item->undirected_edges_list[num_undirected_edges] =
      undirected;
...
void directed_to_undirected(DirectedEdge *directed, UndirectedEdge *undirected,
  ...
  // Add a new directed_edge_index to the directed_edge_indices pointer. This
  // should be the first
...
void append_to_undirected_edges_list(UndirectedEdge **undirected_edges_list,
                                     UndirectedEdge *to_add,
                                     int64_t *num_undirected_edges) {
  // No need to realloc for space since our original alloc should cover
  // everything
...
void append_to_directed_edge_indices(UndirectedEdge *undirected_edge,
                                     int64_t directed_edge_index) {
  // TODO: don't need to realloc if we always know that there will be 2 directed
  // edges per undirected edge. Update this later for performance boosts.
  // TODO: other random performance boost: don't pass int64_ts into function
  // parameters, pass int64_t* instead
  undirected_edge->directed_edge_indices =
      realloc(undirected_edge->directed_edge_indices,
              sizeof(int64_t) * (undirected_edge->num_directed_edges + 1));
  undirected_edge->directed_edge_indices[undirected_edge->num_directed_edges] =
      directed_edge_index;
  undirected_edge->num_directed_edges += 1;
}

// If there already exists neighbor_index within the Node node, then adds
// directed_edge to the list of directed edges. If there doesn't already exist
// neighbor_index within the Node node, then create a new entry into the node's
// neighbors hashmap and add the entry
void add_neighbors_to_node(Node *node, int64_t neighbor_index,
                           DirectedEdge *directed_edge) {
  ...
  if (entry) {
    ...
    entry->directed_edges_list = realloc(
        entry->directed_edges_list,
        sizeof(DirectedEdge *) * (entry->num_directed_edges_in_group + 1));
    entry->directed_edges_list[entry->num_directed_edges_in_group] =
        directed_edge;
    ...
    // The entry doesn't exist, initialize the entry and enter it into our
    // hashmap
...
// Returns a list of LongToDirectedEdgeList pointers which are entries for the
// neighbors of the inputted node
LongToDirectedEdgeList **get_neighbors(Node *node) {
  int64_t num_neighbors = HASH_COUNT(node->neighbors);
  LongToDirectedEdgeList **entries =
      malloc(sizeof(LongToDirectedEdgeList *) * num_neighbors);
