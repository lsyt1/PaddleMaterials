...
class IntegratorPredictor(BasePredictor):
    """Predict atomic flow velocities from molecular-dynamics trajectory samples."""
    ...
    def from_dataset_sample(
        ...
        path = data_path or self.predict_config.get("path")
        index_file = index_file or self.predict_config.get("index_file")
        if path is None or index_file is None:
            raise ValueError("path and index_file must be provided.")
